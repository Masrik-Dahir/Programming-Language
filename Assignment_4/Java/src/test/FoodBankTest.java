...
public class FoodBankTest {
    ...
    public void FoodProducer_ValidObjectTest() {
        ...
        assertSame("When checking the superclass of FoodProducer we",
                c.getSuperclass(), Thread.class);
    ...
    public void FoodConsumer_ValidObjectTest() {
        ...
        //Test subclasses Thread
        assertSame("When checking the superclass of FoodConsumer we",
                c.getSuperclass(), Thread.class);
        //Test parameterized constructor
        try {
            c.getDeclaredConstructor(FoodBank.class);
    ...
    @Test
    //@GradedTest(name="Test if FoodBank's bank instance variable ever drops belowzero with multiple consumer threads", max_score=2.5)
    ...
    private void instanceVariablePrivate(String aField, Object testObject) {
        ...
        try {
            ...
            assertTrue("You must make your instance variables private.",
    ...
    private void instanceVariableCorrectType(String aField, Class<?> aClass,
                                             Object testObject) {
        @SuppressWarnings("rawtypes")
        Class c = testObject.getClass();
        try {
            c.getDeclaredField(aField);
            assertEquals("You must make the speed instance variable of type" +
                    aClass.toString() + ".", aClass, c.getDeclaredField(aField).getType());
        } catch (NoSuchFieldException e) {
            fail("Could not find the " + e.getLocalizedMessage() + " instance variable");
        } catch (Exception e) {
            fail("Something weird went wrong");
    ...
    private void testVariable(String aField, Object testObject, Object expected,
                              String message) {
        @SuppressWarnings("rawtypes")
        Class c = testObject.getClass();
        try {
